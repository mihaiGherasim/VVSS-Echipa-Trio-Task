...
public class TaskRepository {
    ...
    public static String getFormattedInterval(int interval){
        ...
        int timeIndexStart = 0, timeIndexEnd = time.length-1;
        while (time[timeIndexStart] == 0 || time[timeIndexEnd] == 0){
            if (time[timeIndexStart] == 0) timeIndexStart++;
            if (time[timeIndexEnd] == 0) timeIndexEnd--;
        }

        for (int timeIndexBetweenStartAndEnd = timeIndexStart; timeIndexBetweenStartAndEnd <= timeIndexEnd; timeIndexBetweenStartAndEnd++){
            sb.append(time[timeIndexBetweenStartAndEnd]);
            sb.append(time[timeIndexBetweenStartAndEnd] > 1 ? TIME_ENTITY[timeIndexBetweenStartAndEnd]+ "s" : TIME_ENTITY[timeIndexBetweenStartAndEnd]);
            sb.append(" ");
        }
        return sb.toString();
    }
    ...
    public static void rewriteFile(ObservableList<Task> tasksList) {
        ...
        try {
            ...
            log.error("Cannot write task list into file!!!!!!!!!!!");
        }
    }
}
